...
void ShadowShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView**depthMaps, Light** light)
{
	...
	for (int i = 0; i < 2; i++) {
		LightStruct l;
		l.ambient = light[i]->getAmbientColour();
		l.diffuse = light[i]->getDiffuseColour();
		l.direction = light[i]->getDirection();
		l.padding = 0;
		lightPtr->l[i] = l;
	}
